...
data <- read.csv("data/dataset_short_all.csv", row.names = "MS_ID") %>%
...
ggsave("out/plot/age_marker_correlation.pdf", width = 8, height = 8)
...
ggsave("out/plot/cross_correlation.pdf", width = 8, height = 8)
...
ggsave("out/plot/age_model_residuals.pdf", width = 8, height = 8)
...
ggplot(scree_df, aes(x = PC, y = Variance)) +
  ...
  theme_bw()
ggsave("out/plot/scree_biomarkers.pdf", width = 5, height = 4)
...
scores$patient_id <- data$patient_id
...
ggplot(scores, aes(x = PC1, y = PC2, color = cluster)) +
  geom_text_repel(aes(label = patient_id), size = 1.9, vjust = -0.7,
  ...
  theme_bw()
ggsave("out/plot/pca_biplot_kmeans.pdf", width = 7, height = 6)
...
# Show ORIGINAL (not residual) z-scored expression so the biology is readable.
...
mat_z <- t(scale(mat))    
colnames(mat_z) <- data$patient_id

# Compute silhouette on original row order before any reordering.
# scores[, 1:2] drops the cluster/patient_id columns added in step 7.
...
pdf("out/plot/heatmap_clusters.pdf", width = 12, height = 6)
...
data_full <- read.csv("data/dataset_short_all.csv", row.names = "MS_ID") %>%
  ...
  rownames_to_column("patient_id") %>%
  left_join(data %>%
              dplyr::select(patient_id, cluster), by = "patient_id")

# cat_vars  <- c("Sex_F1_M2", "future_PIRA", "previous_PIRA",
#                "RELAPSE_act", "MRI_act", "Inflammatory_act")

cat_vars  <- c("future_PIRA", "previous_PIRA",
               "RELAPSE_act", "MRI_act", "Inflammatory_act")

cluster_colors <- c("Group 1" = "#2196F3", "Group 2" = "#FF9800", "Group 3" = "#F44336")
...
ggsave("out/plot/covariate_categorical.pdf", width = 10, height = 7)
